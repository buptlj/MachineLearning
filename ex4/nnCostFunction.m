...
for i=1:m
   ...
   J = J + y_tmp' * log(a3) + (1-y_tmp)' * log(1-a3);
   delta3 = a3 - y_tmp;
...
%J = J/(-m) + nn_params' * nn_params * lambda/(2*m);
Theta1_tmp = Theta1(:,2:input_layer_size+1);
Theta2_tmp = Theta2(:,2:hidden_layer_size+1);
J = J/(-m) + (sum(sum(Theta1_tmp .^ 2)) +sum(sum(Theta2_tmp .^ 2))) * lambda/(2*m);
